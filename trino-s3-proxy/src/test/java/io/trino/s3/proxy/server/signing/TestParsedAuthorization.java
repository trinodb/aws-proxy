...
public class TestParsedAuthorization
{
    ...
    public void testValues()
    {
        ...
        assertThat(parsedAuthorization).extracting(ParsedAuthorization::accessKey, ParsedAuthorization::region).containsExactly("THIS_IS_AN_ACCESS_KEY", "us-east-1");
        assertThat(parsedAuthorization.signedLowercaseHeaders()).containsExactly("amz-sdk-invocation-id", "amz-sdk-request", "amz-sdk-retry", "content-type", "host", "user-agent", "x-amz-content-sha256", "x-amz-date");
        ...
        assertThat(parsedAuthorization.signedLowercaseHeaders()).isEmpty();
        assertThat(parsedAuthorization.signature()).isEmpty();

        parsedAuthorization = parse("x=y,,,,,,SignedHeaders=");
        assertThat(parsedAuthorization).extracting(ParsedAuthorization::accessKey, ParsedAuthorization::region).containsExactly("", "");
        assertThat(parsedAuthorization.signedLowercaseHeaders()).isEmpty();
        assertThat(parsedAuthorization.signature()).isEmpty();

        parsedAuthorization = parse("x=y,,,,,,SignedHeaders=b;b;b;b;c,,,,,,,,,");
        assertThat(parsedAuthorization).extracting(ParsedAuthorization::accessKey, ParsedAuthorization::region).containsExactly("", "");
        assertThat(parsedAuthorization.signedLowercaseHeaders()).containsExactly("b", "c");
        assertThat(parsedAuthorization.signature()).isEmpty();

        parsedAuthorization = parse("AWS4-HMAC-SHA256 Credential=x/y/z,SignedHeaders=1,Signature=foo");
        assertThat(parsedAuthorization.isValid()).isTrue();
        assertThat(parsedAuthorization).extracting(ParsedAuthorization::accessKey, ParsedAuthorization::region).containsExactly("x", "z");
        assertThat(parsedAuthorization.signedLowercaseHeaders()).containsExactly("1");
