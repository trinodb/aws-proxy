...
public class TrinoStsResource
{
    ...
    @Inject
    public TrinoStsResource(SigningController signingController, StsController stsController)
    {
    ...
    public Response post(@Context ContainerRequest request)
    {
        ...
        SigningMetadata signingMetadata = signingController.validateAndParseAuthorization(request, SigningService.STS, Credentials::emulated, entity);
    ...
    private Response assumeRole(ContainerRequest request, SigningMetadata signingMetadata, Map<String, String> arguments, Optional<byte[]> entity)
    {
        ...
        StsAssumedRole stsAssumedRole = stsController.assumeRole(signingMetadata.credentials().emulated().accessKey(), signingMetadata.session(), roleArn, externalId, roleSessionName, durationSeconds)
                .orElseThrow(() -> {
        ...
        String expiration = signingController.formatInstant(stsAssumedRole.expiration());
        AssumeRoleResult response = new AssumeRoleResult(
                new AssumeRoleResponse.AssumedRoleUser(stsAssumedRole.arn(), stsAssumedRole.roleId()),
                new AssumeRoleResponse.Credentials(stsAssumedRole.emulatedCredential().accessKey(), stsAssumedRole.emulatedCredential().secretKey(), stsAssumedRole.session(), expiration));
